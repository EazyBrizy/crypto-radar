...
async def list_strategy_test_runs(
    ...
    return service.list_runs(user_id=user_id, limit=limit)
...
async def get_strategy_test_run(
    ...
    if run is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Strategy test run not found")
    return run
...
async def get_strategy_test_report(run_id: UUID) -> StrategyTestReportResponse:
    _ = run_id
    raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Strategy test report not found")
