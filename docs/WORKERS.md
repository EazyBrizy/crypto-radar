...
For local dev, `scripts/dev.ps1` starts `strategy-test-worker` as a managed process next to backend and frontend after Alembic migrations. If backend and frontend are started manually, start the worker in its own terminal from `backend` with `..\.venv\Scripts\python.exe -m app.workers.strategy_test_worker`; historical Strategy Testing runs remain `queued` without it. `forward_virtual` also needs scanner/market ticks after the worker starts the run, so scanner-disabled local dev can leave it `waiting_for_market_data`.
