    ...
    def _base_status(
        ...
        *,
        ...
        if no_trade_filter.blocked:
            block_reason = "; ".join(no_trade_filter.blockers)
            if no_trade_filter.hard_block:
                return ("rejected", f"No-trade hard block: {block_reason}")
            return ("ready", f"No-trade block: {block_reason}")
