...
- `POST /api/v1/strategy-tests/runs/estimate` is the backend-owned launch estimate for historical runs. It counts available historical candles from `HistoricalCandleProvider.count_candles(...)`, so totals are deduped by `(exchange, symbol, ts)` and reduced by the backend warmup policy. The response includes per-scenario bars, total bars, size level, and validation warnings for missing market data or raw rows that exceed deduped candles above the backend threshold.
