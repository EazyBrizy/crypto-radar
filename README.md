...
- Режимы demo / virtual / real-testnet / real-mainnet и safety gates:
  `docs/trading-modes-and-safety.md`.
...
При `use_all_symbols=true` scanner использует persisted market universe: Bybit pairs синхронизируются через `/api/v1/market-universe/sync`, доступны лимиты `top_100`, `top_200`, `top_500` и `all`. По умолчанию scanner берет Top 200, добавляет explicit strategy pairs и применяет `MAX_SCANNER_PAIRS`, чтобы защитить локальную нагрузку. Если universe еще не синхронизирован или Bybit REST временно недоступен, приложение использует fallback MVP-список.
...
Режим `real` по умолчанию строит dry-run execution plan и не отправляет ордер на биржу. Live order placement выключен backend-флагами; testnet требует отдельного включения, а mainnet требует дополнительный opt-in:
...
Подробные правила для demo identity, virtual/paper trading, real-testnet, real-mainnet, pending entry reconfirmation, market universe и env-флагов описаны в `docs/trading-modes-and-safety.md`.
...
Безопасные defaults для live trading должны оставаться выключенными:
...
`ENABLE_BYBIT_MAINNET_ORDER_PLACEMENT=true` разрешен только для явно утвержденного mainnet-окружения после testnet-first проверки. Не добавляй реальные exchange secrets в документацию, README, `.env.example` или commit history.
