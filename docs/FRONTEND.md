...
- The Strategy Testing panel may show a frontend-only run-size estimate before launch: scenario count, approximate bars per scenario, approximate total bars, and small/medium/large warning level from the selected date range, pairs, strategies, and timeframes. Large historical runs require an explicit confirmation click before submitting the backend request.
...
- Active historical progress should surface backend `runtime_state` progress fields such as `bars_processed`, `bars_total`, `bars_pct`, `bars_per_second`, `eta_seconds`, `pending_armed`, `pending_entries_count`, `no_entry`, and `filled` when present.
