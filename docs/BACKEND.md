...
- Strategy Testing is the canonical historical backtest pipeline. Legacy `POST /api/v1/backtests/run` is only a compatibility wrapper: it maps the single strategy/pair/timeframe payload to a `historical_backtest` `StrategyTestRunRequest`, enqueues the run, returns `202` with `run_id`, and points clients to `/api/v1/strategy-tests/runs/{run_id}` and `/api/v1/strategy-tests/reports/{run_id}`.
- Legacy `GET /api/v1/backtests/results` reads Strategy Testing reports. Do not add a separate `/backtests` worker, result table, PnL calculator, fee/slippage path, or entry lifecycle.
