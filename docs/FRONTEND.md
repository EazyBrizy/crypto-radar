...
- The Strategy Testing panel must use `POST /api/v1/strategy-tests/runs/estimate` for historical launch estimates. The frontend may display scenario count, backend-counted bars per scenario, total bars, size level, and validation warnings, but it must not estimate historical bars from date range or timeframe formulas. Large historical runs require an explicit confirmation click before submitting the backend request.
- Strategy Testing presets are UI shortcuts only: Smoke narrows to one pair, one timeframe, three days, and `production_like`; Research keeps the selected matrix in `research_virtual`; Forward virtual keeps the selected matrix and switches `test_type` to `forward_virtual`.
- Active historical progress should surface backend `runtime_state` progress fields, not frontend estimates. Use `matrix_bars_processed`, `matrix_bars_total`, and `bars_pct` for whole-matrix progress; use `current_scenario_index`, `current_scenario_key`, `current_scenario_bars_processed`, and `current_scenario_bars_total` for the currently running scenario.
- Historical progress counters should prefer `runtime_state.counters.signals`, `execution_candidates`, `pending_armed`, `pending_entries`, `no_entry`, `filled`, `closed`, `risk_rejections`, and `execution_rejections`. Legacy top-level aliases may be displayed only as fallback.
- Display backend `runtime_state.phase`, `last_progress_at`, `last_heartbeat_at`, and `stale_threshold_seconds` when present. Stale decisions and allowed cancel/run actions still come from the active-run endpoint, not from frontend timers.
- For fronttests, display backend `runtime_state.status`, `runtime_state.last_heartbeat_reason`, and counters such as `processed_ticks`, `processed_signals`, `opened_trades`, `trades_written`, and `metrics_written` as received. Common runtime statuses include `waiting_for_market_data`, `listening`, `processing`, `degraded`, and `cancelled`; the backend owns their meaning. `waiting_for_market_data` means the worker is alive but no matching market tick has reached the run yet.
- Root `/health.strategy_test_worker` is a database lease snapshot for the separate strategy-test worker service. Use it for worker liveness/status chrome only; selected-run UI state still comes from strategy-test run detail and `runtime_state`.
- Treat `runtime_state.pending_entries` as a compact forward pending-entry snapshot for the active panel only. It is capped for terminal entries and is not a full historical list; use `/signals`, `/trades`, and report endpoints for durable history.
