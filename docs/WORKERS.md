...
Historical strategy-test progress is emitted as absolute counters and merged with the completed-scenario summary in `runtime_state.partial_summary`. `bars_processed`, `bars_total`, and `bars_pct` describe matrix-wide deduped candle progress when the runner can count historical candles before execution; `scenario_bars_processed` and `scenario_bars_total` describe the current scenario. `bars_per_second`, `elapsed_ms`, and `eta_seconds` remain runtime estimates from the scenario start. `signals_seen` counts generated strategy signals. `execution_candidates` counts signals selected for execution consideration, including unresolved pending entries. `pending_armed` counts signals armed for historical pending-entry evaluation, while `pending_entries_count` is the current unresolved pending queue size. `entry_touched`/`touched` count signals whose entry zone or fill was reached; `filled` counts opened simulated trades; `closed` counts completed simulated trades. `no_entry` counts signal events that never opened a trade, and `not_selected` is the subset skipped by signal selection. `risk_rejections` and `execution_rejections` keep their existing RiskGate and virtual-execution meanings.
...
`backend/app/workers/strategy_test_worker.py` is the durable strategy-test worker entrypoint. It claims queued `strategy_test_runs` with a PostgreSQL row lock (`FOR UPDATE SKIP LOCKED`), records `worker_id`, increments `worker_attempt`, sets `claimed_at` and `lease_expires_at`, and executes runs through `StrategyTestingService`. Historical backtests run in a worker-managed thread while the worker loop renews `last_heartbeat_at` and the lease independently of progress callbacks.

The Docker Compose service is `strategy-test-worker` in `infra/docker-compose.yml` under the `app` and `dev` profiles. It builds the backend image and runs `python -m app.workers.strategy_test_worker` against PostgreSQL, Redis, NATS, and ClickHouse. The strategy smoke starts it with `docker compose -f infra/docker-compose.yml --profile dev up -d --build --force-recreate backend-dev strategy-test-worker`; `backend-dev` serves the API while this worker owns queued historical runs, forward run startup, and forward heartbeats.

For local dev, `scripts/dev.ps1` starts `strategy-test-worker` as a managed process next to backend and frontend after Alembic migrations. If backend and frontend are started manually, start the worker in its own terminal from `backend` with `..\.venv\Scripts\python.exe -m app.workers.strategy_test_worker`. Historical backtests require this worker and remain `queued` without it. `forward_virtual` requires this worker plus scanner/market data: the worker starts and heartbeats the run, while scanner ticks/signals drive the forward runtime. With `scripts/dev.ps1 -NoScanner`, `forward_virtual` can correctly remain `waiting_for_market_data` until scanner ticks are flowing.

The durable worker also starts queued `forward_virtual` runs and calls the forward runtime heartbeat for already running forward tests. `backend/app/workers/forward_strategy_test_worker.py` remains a runtime adapter for tests or market-data integrations that call `process_market_tick()` / `process_strategy_signal()` directly; it is not started from FastAPI lifespan. Root `/health` exposes `strategy_test_worker` from the database lease fields (`worker_id`, `worker_attempt`, `claimed_at`, `lease_expires_at`, `last_heartbeat_at`) so the API can report the separate worker service without owning the loop.

The FastAPI `/health` root endpoint reports no in-process forward worker by default (`forward_strategy_test_running=false`). Use the strategy-test run detail for UI-facing run state such as `runtime_state.status`, `runtime_state.last_heartbeat_reason`, `last_heartbeat_at`, and stale policy fields.

Expired leases are recovered by the durable worker: queued runs are requeued by clearing the lease, running runs are marked `failed` with a worker-lease-expired reason, and stale `stopping` runs are marked `cancelled` so they no longer block new launches.

Forward pending entries are stored as a compact runtime snapshot, not as an analytics ledger. The current field is `runtime_state.pending_entries`; older design notes may refer to the same snapshot as `runtime_state.forward_pending_entries`. Active pending entries are retained, while terminal entries (`filled`, `expired`, `blocked`, `cancelled`, `failed`) are capped to the most recent 200. Use strategy-test signal events and trades for durable reporting; do not treat the runtime pending-entry snapshot as complete history. `runtime_state.processed_signal_keys` is a capped idempotency guard for forward scanner signal processing, not a report source.
